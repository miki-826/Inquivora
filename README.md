...
- [x] Phase 7: 検索・品質（全文検索＝SQLite FTS5 trigramで日本語部分一致・2文字以下はLIKEフォールバック§15.2／ファイル・議事録・タスク・予定を横断検索し種別で絞り込み・snippet表示・結果クリックで対象へ遷移／`search_global`・`search_reindex_workspace`§17.7／タスク・予定・会議・ファイルの変更で索引を即時更新§15.3／クラッシュ復旧＝起動時に中断された処理中ジョブをpendingへ戻す）— v0.1.0-alpha.11。**残: 自動更新（updaterプラグイン）は署名鍵とendpoint設定がユーザー準備待ちのためPhase 7時点では未組込み、E2Eは単体テスト（Rust267・フロント108・C#59）でカバー**
