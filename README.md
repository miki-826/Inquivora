...
- 文字起こし軽量化・議事録/カレンダー修正 — v0.1.0-alpha.22: 録音中の音量メーターをセグメント一覧から分離し、確定セグメントをメモ化して文字起こし中の再描画負荷を軽減。選択中の会議と中央表示・要約対象を常に一致させ、表示と別の会議を要約する不具合を修正。ペイン開閉ボタンと下部操作ボタンの重なりを解消し、カレンダーで選択したタスクを右の予定詳細でそのまま編集できるように変更
- 議事録要約・録音保存・分割操作改善 — v0.1.0-alpha.21: 要約生成後に日付だけなどの汎用的な会議名をAI生成タイトルへ自動更新し、要約が文字起こし本文の内容を対象にするよう明確化。録音全体をWAVで保存する「録音を保存」を追加。設定の用途別モデル設定を「使用/予備」の2行カード構成にして余白を確保し、タブのドラッグ&ドロップでエディタを左右分割表示できるように改善
